...
def query_graph(store: GraphStore, question: str, provider: AIProvider | None = None) -> dict[str, Any]:
    ...
    if not selected_evidence:
        ...
        if not citations or any(value < 1 or value > len(selected_evidence) for value in citations):
            excerpts = " ".join(
                f"[E{index}] {row['document_title']}: {row['excerpt']}"
                for index, row in enumerate(selected_evidence[:3], start=1)
            )
            answer = f"The generated synthesis was rejected because it lacked valid evidence citations. Verified evidence: {excerpts}"
            caveats = ["Safe extractive fallback shown; no uncited Gemini claims were returned."]
        else:
